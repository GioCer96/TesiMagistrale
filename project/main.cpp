...
int main(int argc, char **argv) {
	...
	*/
	...
	//NEW VARIABLES
	int final_scores[query_seqs_len];
	int second_final_scores[query_seqs_len];

	int query_batch_starts[query_seqs_len];
	int target_batch_starts[target_seqs_len];
	int query_batch_ends[query_seqs_len];
	int target_batch_ends[target_seqs_len];
	int second_query_batch_ends[query_seqs_len];
	int second_target_batch_ends[target_seqs_len];
			...
			while (gpu_batch_arr_idx < gpu_storage_vecs[omp_get_thread_num()].n) {//loop through all the streams and print the results
				...
				if (gasal_is_aln_async_done(gpu_batch_arr[gpu_batch_arr_idx].gpu_storage) == 0) {
					...
					for (int i = gpu_batch_arr[gpu_batch_arr_idx].batch_start; j < gpu_batch_arr[gpu_batch_arr_idx].n_seqs_batch; i++, j++) {
						...
						final_scores[i] = (gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->host_res->aln_score[j];
								...
								|| args->algo > SEMI_GLOBAL))
						{
							...
							query_batch_starts[i] = (gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->host_res->query_batch_start[j];
							target_batch_starts[i] = (gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->host_res->target_batch_start[j];
						}
						...
						if (args->algo != GLOBAL)
						{
							...
							query_batch_ends[i] = (gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->host_res->query_batch_end[j];
							target_batch_ends[i] = (gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->host_res->target_batch_end[j];
						}
						...
						if (args->secondBest)
						{
							...
							second_final_scores[i] = (gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->host_res_second->aln_score[j];
							...
							second_query_batch_ends[i] = (gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->host_res_second->query_batch_end[j];
							second_target_batch_ends[i] = (gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->host_res_second->target_batch_end[j];
						}
						...
						if (args->start_pos == WITH_TB) {
							...
							std::cout << count << op;
							//std::string cigar = count + op;
							//std::cout << "PROVA CIGAR" << cigar;
						}
						std::cout << std::endl;
					}
					}
	...
	*/
	...
	//SAM FILE CREATION
        SamFile samout;
        samout.OpenForWrite("./prova.sam");
        SamFileHeader samHeader;

	std::string hd = "@HD\tVN:1.0\tSO:queryname\n";

	for(int i = 0; i<target_headers.size(); i++) {
		std::string len = std::to_string(target_seqs[i].length());
		std::string line = ("@SQ\tSN:" + target_headers[i] + "\tLN:" + len + "\n");
		hd = hd + line;
	}
	...
	std::cerr << std::endl << "HEADER FLAG: " << flag << std::endl;
        samout.WriteHeader(samHeader);

	for(int i = 0; i<target_headers.size(); i++) {
                SamRecord samRecord;
                samRecord.setReadName(query_seqs[i].c_str());
                if(final_scores[i] < 15)
                        samRecord.setFlag(4);
                else
                        samRecord.setFlag(2);
                samRecord.setReferenceName(samHeader, target_headers[i].c_str());
		samRecord.set1BasedPosition(query_batch_starts[i]);
		samRecord.setMapQuality(255);
		std::string cig = "*";
		samRecord.setCigar(cig.c_str());
		std::string rnext = "*";
		samRecord.setMateReferenceName(samHeader, rnext.c_str());
		samRecord.set1BasedMatePosition(0);
		int tlen = query_batch_ends[i] - query_batch_starts[i] + 1;
		samRecord.setInsertSize(tlen);
		samRecord.setSequence(query_headers[i].c_str());
		std::string qual = "*";
		samRecord.setQuality(qual.c_str());
