...
int main(int argc, char **argv) {
	...
	*/
	...
	std::string s = std::to_string(target_seqs_len);
	const char* ts_len = s.c_str();
	char* hd = "@HD\tVN:1.0\tSO:queryname\n";
	for(int i = 0; i<query_headers.size(); i++) {
		char* line = "@SQ\tSN:" + query_headers[i] "\tLN:";
	char temp[100];
	strcpy(temp, hd);

	const char* header = strcat(temp, ts_len);

        bool flag = samHeader.addHeader(header);
